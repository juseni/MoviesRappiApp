...
class RatedMoviesRepositoryImplTest {

    companion object {
        private const val QUERY_REQUEST = "vote_average.desc"
    ...
    @Mock
    private lateinit var dao: RatedMoviesDao
    @Mock
    private lateinit var networkHandler: NetworkHandler



    private lateinit var ratedRepositoryImpl: RatedMoviesRepositoryImpl

    @Before
    fun setup() {
        given { networkHandler.isConnected() }.willReturn(true)
        ratedRepositoryImpl = RatedMoviesRepositoryImpl(gson, api, dao, networkHandler)
    }

    @Test
    fun `it should execute get rated movies from repository`() {
        ...
        val response = ratedRepositoryImpl.getRatedMovies(PAGE_DEFAULT).test().assertComplete().values()[0]
    ...
    fun `it should get an Exception`() {
        ...
        val testObserver = ratedRepositoryImpl.getRatedMovies(PAGE_DEFAULT).test().assertError(Exception::class.java)
